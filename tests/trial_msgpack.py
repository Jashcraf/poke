...
def trace_polarized_rayfront():
    ...
    wavelength = 1.65
    surflist = [s1,s2,s3]
    rf = Rayfront(nrays,wavelength*1e-6,1.2,.08)
    rf.as_polarized(surflist)
    rf.trace_rayset(pth)

    # save
    write_rayfront_to_serial(rf,f'hst_rayfront_aspolarized_{nrays}rays_{wavelength}um')

def read_rayfronts():

    pth_beams = 'hst_rayfront_asbeamlets_32rays_1.65um.msgpack'
    pth_polar = 'hst_rayfront_aspolarized_32rays_1.65um.msgpack'
